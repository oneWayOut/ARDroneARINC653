...
void actuators_ardrone_init(void)
{
  ...
  //all leds green
  actuators_ardrone_set_leds(LEDGREEN, LEDGREEN, LEDGREEN, LEDGREEN);
  sleep(1);
  actuators_ardrone_set_leds(LEDORANGE,LEDORANGE,LEDORANGE,LEDORANGE);
}
...
/**
 * Write LED command
 * cmd = 011r rrrx xxxg gggx
 */
void actuators_ardrone_set_leds(uint8_t led0, uint8_t led1, uint8_t led2, uint8_t led3)
{
  uint8_t cmd[2];
  cmd[0]=0x60 | ((led3&1)<<4) | ((led2&1)<<3) | ((led1&1)<<2) | ((led0&1)<<1);
  cmd[1]=0x00 | ((led3&2)<<3) | ((led2&2)<<2) | ((led1&2)<<1) | (led0&2);
  write(mot_fd, cmd, 2);
}
